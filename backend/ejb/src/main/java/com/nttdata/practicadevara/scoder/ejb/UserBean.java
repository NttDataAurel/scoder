...
public class UserBean {
    ...
    private List<UserDto> toDto(List<UserDbEntity> list) {
        if (list != null) {
            return list.stream().map(e -> new UserDto(e.getId(), e.getName(), e.getSurname(), e.getAddress(), e.getPhone(), e.getEmail(), e.getFilename(), e.getFileData(), e.getState())).collect(Collectors.toList());
        }
    ...
    private UserDto toDto(UserDbEntity e) {
        if (e != null) {
            return new UserDto(e.getId(), e.getName(), e.getSurname(), e.getAddress(), e.getPhone(), e.getEmail(), e.getFilename(), e.getFileData(), e.getState());
        }
        return null;
    }
